...
Xively stopped their activities for "free" customers :-(
Switched to ThingSpeak ( http://www.thingspeak.com )

- Created a channel Called WaterTank. Channel ID is 501076
- Created a field called WaterLevel (no. of the is 1. Each channel can have up to 8 fields).
===========================================================================================================================
...
IMPORTANT: to make it work, the "Ethernet" directory form the extracted fiels had to be copied into the
...
const char * myWriteAPIKey = "CS89U8CWIQTOMAEC";
...
void setup() {
  //set pin 8 to high before anything else. This pin will be used to reset the Arduino
  digitalWrite(8, HIGH);
  pinMode(8, OUTPUT);
...
void loop() {  
  ...
  if (!client.connected() && lastConnected) {
   ...
   // Send the measured tank value to ThingSpeak
   sendData2Cloud(cm);
...
// send the supplied value to ThingSpeak, printing some debug information as we go
void sendData2Cloud(int sensorValue) {
